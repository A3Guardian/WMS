...
function formatAction(action, t) {
    const labels = {
        created: t("inventory.activity.actions.created"),
        updated: t("inventory.activity.actions.updated"),
        adjusted: t("inventory.activity.actions.adjusted"),
        deleted: t("inventory.activity.actions.deleted"),
    };
...
export default function InventoryActivityModal({ isOpen, onClose, inventory }) {
    ...
    const productName =
        ...
        t("inventory.activity.inventoryNumber", { id: inventory.id });
    const location =
        [inventory.deposit?.name, inventory.shelf?.name]
            .filter(Boolean)
            .join(" – ") || t("common.dash");
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl p-6 w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col z-50">
                    <Dialog.Title className="text-xl font-bold mb-1 flex items-center gap-2">
                        ...
                        {t("inventory.activity.title")}
                    </Dialog.Title>
                    <p className="text-sm text-gray-600 mb-4">
                        {productName}
                        {location !== t("common.dash") && (
                        ...
                        {error && (
                            <p className="text-red-600 text-sm">
                                {error?.response?.data?.message ||
                                    error?.message ||
                                    t("inventory.activity.loadFailed")}
                            </p>
                        )}
                        {!loading && !error && (!logs || logs.length === 0) && (
                            <p className="text-gray-500 text-sm">
                                {t("inventory.activity.empty")}
                            </p>
                        )}
